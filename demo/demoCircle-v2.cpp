...
#endif
  std::cout << "beta_x:" << privpf.beta_x << "beta_y: " << privpf.beta_y  <<"beta_z: " << privpf.beta_z<<std::endl;
...
void set_node_location(Prover &p, Verifier &v, CryptoPP::RandomNumberGenerator &rng, const int xn, const int yn, const int zn) {
	...
	p.pubi.su = scomm;
	v.pubi.su = scomm;
	std::cout << "p.pubi.su " << p.pubi.su << "p.privi.r" << p.privi.r  << std::endl;
#ifdef DBG_LOCATIONS
...
//  double xl=47.1667111, yl=8.6166111, zl=100.111122;
  double xl = 30.534246, yl = 114.36626, zl = 34.9;
  long RR=1000;
...
//  printf("xl :  %.7lf   yl:%.7lf    zl:%.7lf\n",xl,yl,zl);
//  pauseLines(6);
  // xn = 47.1666 (grad lalitute) yn = 8.5161 (grad longtitude), zn = 425 (meters)
//  double xn=47.1667111, yn=8.6166112, zn=425.3431124;
  double xn = 30.533632, yn = 114.366721, zn = 34.9;
...
//               "Format: Lat/Long coordinates (x,y,z), pocket_address\n Example: 2,1,3,UUID" << std::endl;
  ...
  */
  //std::cout << " Pocket XN = " << xn << " YN = " << yn << " ZN = " << zn << std::endl;
	...
	set_node_location(P, V,  randPool, gcs.get_coord_x(),  gcs.get_coord_y(),  gcs.get_coord_z());
//  set_node_location(P, V, 1000000 * gcs.get_coord_x(), 1000000 * gcs.get_coord_y(), 1000 * gcs.get_coord_z());
